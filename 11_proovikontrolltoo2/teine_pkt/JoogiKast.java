...
public class JoogiKast {
    ...
    public double getMass(int jookideArv) {
        if(jookideArv > pesad) {
            jookideArv = pesad;
            System.out.println("Jookide arv oli suurem kui peasde arv, " + "votan kasutusele " + pesad + "jooki.");
        }
        if (joogiPudel == null) {
            return kastiMass;
        } else {
    ...
    public double getHind(int jookideArv) {
        if(jookideArv > pesad) {
            jookideArv = pesad;
            System.out.println("Jookide arv oli suurem kui pesade arv, " + " votan kasutusele " + pesad + "jooki.");
        }
        if (joogiPudel == null) {
            return kastiHind;
        } else {
    ...
    public int villiKastiPudelid(Joogivaat joogivaat) {
        ...
        while (joogivaat.liitritesJooki >= joogiPudel.maht) {
            ...
            loendur++;
            if (loendur == pesad) {
                System.out.println("Lopetasin villimise, kuna sai tais");
                break;
            }
        }
